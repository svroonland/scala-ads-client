  ...
  override def spec =
    suite("ADS client")(
      testM("read many variables at once") {
        for {
          var1r <- AdsClient.varHandle(variableList).use(_.read)
      ...
      testM("write many variables at once") {
        for {
          _ <-
            AdsClient
              .varHandle(variableList)
              .use(_.write(8.toShort :: "New value !! Yes" :: 10.toShort :: MyStruct(103, true) :: HNil))
        } yield assertCompletes
      },
      testM("give an error when attemptign to read a variable that does not exist") {
        ...
        for {
          result <- AdsClient.varHandle(variableList).use(_.read).run
