...
class VariableList[T <: HList] private (
  ...
  def +[U, TU <: HList, KLen <: Nat](varName: String, codecU: Codec[U])(implicit
    prepend: Prepend.Aux[T, U :: HNil, TU],
    length: Length.Aux[T, KLen],
    split: Split.Aux[TU, KLen, T, U :: HNil]
  ): VariableList[TU] =
    new VariableList(
      variables = variables :+ varName,
      sizes = sizes :+ codecU.sizeBound.exact
        .getOrElse(throw new IllegalArgumentException("Expected codec with upper bound")),
